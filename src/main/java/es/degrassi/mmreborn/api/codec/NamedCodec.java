...
public interface NamedCodec<A> {
  ...
  default void toNetwork(A input, FriendlyByteBuf buf) {
    ...
    if(tag instanceof CompoundTag compoundTag)
      ...
      compoundTag.put("modularmachineryreborn$special_nbt_key", tag);
      buf.writeNbt(compoundTag);
    }
  }
  ...
  default A fromNetwork(FriendlyByteBuf buf) {
    ...
    DataResult<A> result;
    if(tag != null && tag.contains("modularmachineryreborn$special_nbt_key"))
      result = read(NbtOps.INSTANCE, tag.get("modularmachineryreborn$special_nbt_key"));
    else
