...
public class DataType<D extends IData<T>, T> {
     ...
     * @param reader A {@link BiFunction} that retrieve the object to sync from vanilla {@link RegistryFriendlyByteBuf}.
     * @param <T> The type of the object to sync.
     * @param <D> The {@link IData} used to sync the object.
     */
    public static <T, D extends IData<T>> DataType<D, T> create(Class<T> type, BiFunction<Supplier<T>, Consumer<T>, ISyncable<D, T>> builder, BiFunction<Short, RegistryFriendlyByteBuf, D> reader) {
     ...
     */
    public D readData(short id, RegistryFriendlyByteBuf buffer) {
    ...
    public static <T> ISyncable<IData<T>, T> createSyncable(Class<T> type, Supplier<T> supplier, Consumer<T> consumer) {
        Optional<DataType<IData<T>, T>> dataType =
            ModularMachineryReborn.dataRegistrar().entrySet().stream().filter(entry -> entry.getValue().type == type).map(entry -> (DataType<IData<T>, T>)entry.getValue()).findFirst();
